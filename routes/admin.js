...
router.post('/admin/create-event', adminAuth, async (req, res) => {//Admin create event route
  ...
  try {
    // Convert local date/time to UTC date conversion issue due to MongoDB
...
// Get specific event details to edit
...
router.post('/admin/manage-event/:id', adminAuth, async (req, res) => {
  ...
  try {
    ...
    // Convert date to UTC mongoDB issue
    const localDate = new Date(dateTime);
    const utcDate = new Date(localDate.getTime() - localDate.getTimezoneOffset() * 60000);
